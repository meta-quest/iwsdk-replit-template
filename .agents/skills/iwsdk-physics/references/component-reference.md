...
The three ECS components that make up IWSDK physics: `PhysicsBody` (how a body
moves), `PhysicsShape` (its collision volume and material), and
`PhysicsManipulation` (imperative forces and impulses).
...
| `density`     | `Float32`          | `1.0`       | Mass density in kg/m³ (SI, passed straight to Havok). Higher = heavier. The `1.0` default is a nominal unit density ≈ air, **not** water — see [Material Tuning Guide](tuning-and-config.md#material-tuning-guide). |
